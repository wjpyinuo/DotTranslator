...
export async function eventRoutes(app) {
    ...
    app.post('/events', async (request, reply) => {
        ...
        const instanceId = request.headers['x-instance-id'] || events[0]?.payload.instanceId;
        if (!instanceId) {
      ...
      `, [instanceId, firstEvent?.version || 'unknown', firstEvent?.os || 'unknown',
            ...
            for (const event of events) {
                ...
                values.push(instanceId, event.type, event.payload.feature || null, JSON.stringify(event.payload.metadata || {}), event.timestamp);
            }
            if (placeholders.length > 0) {
                await pool.query(`INSERT INTO events (instance_id, event_type, feature, metadata, client_ts) VALUES ${placeholders.join(', ')}`, values);
            }
            // Redis 实时更新
