...
export async function checkAlerts() {
    const pool = getPool();
    const { rows: rules } = await pool.query('SELECT * FROM alert_rules WHERE is_enabled = TRUE');
    for (const rule of rules) {
        try {
            ...
            if (triggered) {
                await pool.query('UPDATE alert_rules SET last_triggered = NOW() WHERE id = $1', [rule.id]);
...
async function getMetricValue(metric) {
    switch (metric) {
        ...
        case 'dau_drop_pct': {
            ...
            const { rows } = await pool.query('SELECT dau FROM daily_metrics WHERE date IN ($1, $2) ORDER BY date DESC', [today, yesterday]);
            if (rows.length < 2)
                return null;
